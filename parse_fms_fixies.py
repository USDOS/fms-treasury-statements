...
def table_2_merge_and_drop_rows(i):
	two_line_ind = fwfs[i][fwfs[i]['item'].notnull() & fwfs[i]['today'].isnull() & fwfs[i]['mtd'].isnull() & fwfs[i]['fytd'].isnull() & ~fwfs[i]['item'].str.endswith(':') & ~fwfs[i]['item'].str.endswith('.')].index
...
f_names = f_names_all[-250:]
#f_names = ['12060101.txt']
for f_name in reversed(f_names):
	...
	temp_pages = re.split(r'\d.*DAILY TREASURY STATEMENT.*PAGE:\s+\d\s{2}', f)
	pages = []
	# get rid of empty lines
	for page in temp_pages:
		#pattern = re.compile('___________________________________________________________________________________________')
	...
	if date > datetime.date(2013, 1, 3):
    ...
    #                                                                to date      to date
	# ____________________________________________________________________________________________
	fwfs[2] = pd.read_fwf(StringIO.StringIO(str(pages[2])), skiprows=11, skipfooter=2, thousands=',',
    ...
    #                                                                to date      to date
	#____________________________________________________________________________________________
	fwfs[3] = pd.read_fwf(StringIO.StringIO(str(pages[3])), skiprows=9, skipfooter=2, thousands=',',
		colspecs=[(0,51), (51,65), (65,79), (79,92)],
		names=['item', 'today', 'mtd', 'fytd'],
		converters={'item':clean_text, 'today':clean_text, 'mtd':clean_text, 'fytd':clean_text})
